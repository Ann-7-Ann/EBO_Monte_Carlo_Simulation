...
class LensedMirror(OpticalElement):
    ...
    def __init__(self, p1: Vector, p2: Vector, bulge: float = 40.0, spherical: bool = False):
        self.p1 = p1
        self.p2 = p2
        self.bulge = float(bulge)
        # If True, apply paraxial focusing in the out-of-plane axis (2.5D).
        # This approximates a spherical mirror (focus = R/2).
    ...
    def get_params_str(self):
        ...
        return [
            ...
            f"bulge={self.bulge:.2f}px, radius={radius_str}px, spherical={self.spherical}"
        ]
    ...
    def interact(self, ray, hit):
        ...
        # --- 2.5D out-of-plane focusing (optional) ---
        # A spherical mirror focuses equally in both transverse axes. In paraxial
        # optics the reflection changes slope by: theta_out = theta_in - 2*y/R.
        # Here y corresponds to the out-of-plane coordinate `u` (pixels).
        if getattr(self, "spherical", False) and hasattr(ray, "u") and hasattr(ray, "du"):
            R = float(self._radius) if self._radius else 0.0
            if abs(R) > 1e-12:
                try:
                    ray.du = float(ray.du) - 2.0 * float(ray.u) / R
                except Exception:
                    pass
