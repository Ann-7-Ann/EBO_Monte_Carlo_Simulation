...
class FiberSource:
    ...
    def __init__(
        ...
        total_power=1.0,
        wavelength = 1.3,         
        n_out=1.4898             # Refractive index of the surrounding medium
    ...
    def emit(self, num_rays=110, ray_id_start=0, rng: random.Random | None = None, um_per_px: float = 1.0):
        ...
        waist_um = self.mfd / 2        # meters
        theta_div = self.wavelength / (math.pi * waist_um)  #angular standard deviation

        # Transverse sigma for sampling ray starting points (pixels)   
        ...
        for i in range(num_rays):
            # Gaussian transverse offsets in the two transverse axes:
            # - in-plane offset y0 affects the 2D position
            # - out-of-plane offset u is kept in Ray.u (pixels)
            u_pos_px = (waist_um * 0.5) / float(um_per_px)
            # Angular spreads (in-plane + out-of-plane)
            theta_spread = rng.gauss(0.0, theta_div)
            theta_u = rng.gauss(0.0, theta_div)

            # Launch angle (paraxial) includes cleave angle and small spread
            theta = self.angle + self.cleave_angle + theta_spread

            theta_out = math.asin(self.n_core / self.n_out * math.sin(theta))
