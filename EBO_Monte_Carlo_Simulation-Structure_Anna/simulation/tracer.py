...
def trace(ray, scene, screen, um_per_px: float = 1.0, draw: bool = True):
    ...
    for _ in range(MAX_BOUNCES):
        ...
        if kind == "boundary":
            ...
            new_dir, _did_reflect = refract_or_reflect(direction, normal, m_before.n, m_after.n)
            direction = new_dir
...
def trace_rays(rays, scene, screen, um_per_px: float = 1.0, draw: bool = True):
    for ray in rays:
        trace(ray, scene, screen, um_per_px=um_per_px, draw=draw)