...
from simulation.monte_carlo import run_monte_carlo, DEFAULT_SPECS
...
draw_rays_cap = DRAW_RAYS_CAP_DEFAULT
...
def _build_reference_layout(cfg_override: dict | None = None, sampled_params: dict | None = None):
    """(Re)build the reference layout.

    - cfg_override overrides internal geometry/indices used by the template.
    - sampled_params holds Monte Carlo parameters (MFD, offsets, etc.) used for sources.
    """
    ...
    # --- Parameters (nominal defaults; overridden for Monte Carlo) ---
    cfg = {
        # Refractive indices
        "n_fiber": 1.4527,
        "n_adh": 1.4898,
        ...
        # Extinction (ferrule) in 1/um (table uses 1/mm: 0.008863 -> 8.863e-6 1/um)
        "alpha_ferrule_per_um": 0.008863 / 1000.0,
    }
    if cfg_override:
    ...
    # --- Fiber region with a slanted facet (~cleave_deg from perpendicular) ---
    ...
    adh_pts_1 = [
        ...
        Vector(x_adh_r, y0 + fiber_h + 100),
        Vector(x_adh_l, y0 + fiber_h + 100),
    ]
    adh_pts_2 = [
        Vector(x_adh_r + 190, y0 + 270),
        Vector(x_adh_r + cfg["adh_len_px"] + 230, y0 + 270),
        Vector(x_adh_r + cfg["adh_len_px"] + 230, y0 + fiber_h + 410),
        Vector(x_adh_r + 190, y0 + fiber_h + 410),
    ]
    scene.add(MediaPolygon(adh_pts_1, n=cfg["n_adh"], alpha_per_um=0.0, name="adhesive"))
    scene.add(MediaPolygon(adh_pts_2, n=cfg["n_adh"], alpha_per_um=0.0, name="adhesive"))

    # --- Ferrule region ---
    x_ferr_l = x_adh_r - 10
    x_ferr_r = x_ferr_l + cfg["ferr_len_px"]
    ferr_top = y0 - 60
    ferr_bottom = y0 + fiber_h + 160
    ferr_pts_1 = [
        Vector(x_ferr_l, ferr_top),
        Vector(x_ferr_r, ferr_top),
    ...
    ferr_pts_2 = [
        Vector(x_ferr_l, ferr_bottom),
        Vector(x_ferr_r, ferr_bottom),
        Vector(x_ferr_r, ferr_bottom + fiber_h + 220),
        Vector(x_ferr_l, ferr_bottom + fiber_h + 220),
    ]
    scene.add(MediaPolygon(ferr_pts_1, n=cfg["n_ferr"], alpha_per_um=cfg["alpha_ferrule_per_um"], name="ferrule_1"))
    scene.add(MediaPolygon(ferr_pts_2, n=cfg["n_ferr"], alpha_per_um=cfg["alpha_ferrule_per_um"], name="ferrule_2"))

    # --- Source (FiberSource) ---
    # Use Monte Carlo parameters if present; otherwise keep nominal.
    mfd_um = float(sampled_params.get("launch_MFD_1_um", 9.232917))
    clad_um = float(sampled_params.get("launch_cladding_dia_1_um", 124.9329))
    core_offset_um = float(sampled_params.get("core_clad_offset_launch_1_x_um", 0.0))
    angle_ped_deg = float(sampled_params.get("angle_pedestal_ferrule1_deg", 0.0))
    wavelength_um = float(sampled_params.get("wavelength_um", 1.31))

    um_per_px = float(interaction.um_per_px) if interaction.um_per_px else 1.0
    core_offset_px = core_offset_um / um_per_px

    bx = x_fiber_l + 80
    by = y0 + fiber_h * 0.5

    # NOTE: FiberSource currently applies an exit refraction using n_out.
    # In this template we want the medium boundary to handle refraction, so we set
    # n_out == n_core here (no pre-refraction at emission).
    scene.add(
        FiberSource(
            bx,
            by,
            mfd=mfd_um,
            cladding_diameter=clad_um,
            n_core=cfg["n_fiber"],
            cleave_angle=cfg["cleave_deg"],
            core_offset=(0.0, core_offset_px),
            angle_deg=angle_ped_deg,
            total_power=1.0,
    ...
    # --- Input detector ---
    in_det_x = bx + 35
    scene.add(Detector(Vector(in_det_x, by - 70), Vector(in_det_x, by + 70), label="Input"))

    # # --- Groove end wall as a mirror (uses angle_wall1_y_deg if provided) ---
    # wall_tilt = float(sampled_params.get("angle_wall1_y_deg", -cfg["wall_tilt_deg"]))
    # wall_tilt = abs(wall_tilt)
    # wall_center = Vector(x_fiber_r + 45, y0 + fiber_h + 35)
    # wall_L = 150.0
    # wall_a = math.radians(90.0 + wall_tilt)  # near-vertical, tilted
    ...
    # --- Tilted lensed mirrors (top + bottom) ---
    # Optional offsets from MC table (um) are applied as px via um_per_px.
    um_per_px = float(interaction.um_per_px) if interaction.um_per_px else 1.0
    off_lm_x = float(sampled_params.get("offset_lens1_x_um", 0.0)) / um_per_px
    off_lm_y = float(sampled_params.get("offset_lens1_y_um", 0.0)) / um_per_px

    lm_center = Vector(x_ferr_l + 110 + off_lm_x, y0 + 40 + off_lm_y)
    lm_L = 180
    ...
    scene.add(
        LensedMirror(
            Vector(lm_center.x - mx1, lm_center.y - my1),
            Vector(lm_center.x + mx1, lm_center.y + my1),
            bulge=-112.0,
            spherical=True,
        )
    ...
    offset_from_edge = ferr_bottom - y0 - 40
    lm_center_bottom = Vector(x_ferr_l + 110 + off_lm_x, ferr_bottom + offset_from_edge - off_lm_y)
    lm_a_bottom = math.radians(180 + cfg["lensed_mirror_deg"])
    mx1_bottom = math.cos(lm_a_bottom) * (lm_L * 0.5)
    my1_bottom = math.sin(lm_a_bottom) * (lm_L * 0.5)
    scene.add(
        LensedMirror(
            Vector(lm_center_bottom.x - mx1_bottom, lm_center_bottom.y - my1_bottom),
            Vector(lm_center_bottom.x + mx1_bottom, lm_center_bottom.y + my1_bottom),
            bulge=-112.0,
            spherical=True,
        )
    ...
    # --- Mirrored bottom fiber block (visual) ---
    mirror_x = x_ferr_l + 110
    x_fiber_l_bottom = 2 * mirror_x - x_fiber_r
    x_fiber_r_bottom = 2 * mirror_x - x_fiber_l
    y0_bottom = 2 * ferr_bottom - y0
    fiber_h_bottom = fiber_h
    dx_bottom = math.tan(math.radians(cfg["cleave_deg"])) * fiber_h_bottom
    fiber_pts_bottom = [
        Vector(x_fiber_r_bottom, y0_bottom),
        Vector(x_fiber_l_bottom - dx_bottom * 0.5, y0_bottom),
        Vector(x_fiber_l_bottom + dx_bottom * 0.5, y0_bottom - fiber_h_bottom),
        Vector(x_fiber_r_bottom, y0_bottom - fiber_h_bottom),
    ]
    scene.add(MediaPolygon(fiber_pts_bottom, n=cfg["n_fiber"], alpha_per_um=0.0, name="fiber_bottom"))

    # --- Output detector ---
    det_y = ferr_bottom - 30
    scene.add(Detector(Vector(lm_center.x - 160, det_y), Vector(lm_center.x + 220, det_y), label="Output"))

    template_last_msg = "Reference layout loaded (MC-ready: indices, cleave, adhesive length, ferrule length, offsets)."


def cb_load_reference_layout():
    # Nominal build
    _build_reference_layout()
...
def set_draw_cap_from_field(v):
    global draw_rays_cap
    # Keep it reasonable; 0 means "draw all".
    draw_rays_cap = int(max(0, round(v)))
...
# --- Monte Carlo callbacks / setters ---

def set_mc_trials_from_field(v):
    global mc_trials
    mc_trials = int(max(1, round(v)))


def set_mc_rays_cap_from_field(v):
    global mc_rays_cap
    # 0 means "no cap"
    mc_rays_cap = int(max(0, round(v)))


def set_mc_base_seed_from_field(v):
    global mc_base_seed
    mc_base_seed = int(round(v))


def set_mc_sigma_scale_from_field(v):
    global mc_sigma_scale
    mc_sigma_scale = float(max(0.0, v))


def cb_run_monte_carlo():
    global mc_last_msg
    try:
        exports_dir = os.path.join(os.path.dirname(__file__), "exports")
        um = float(interaction.um_per_px) if interaction.um_per_px else 1.0

        def _build_for_params(sampled: dict[str, float]):
            # Map sampled table parameters into template config where applicable.
            cfg_override: dict = {}
            if "n_adhesive1" in sampled:
                cfg_override["n_adh"] = max(1.0, float(sampled["n_adhesive1"]))
            if "cleave_angle_launch_1_deg" in sampled:
                cfg_override["cleave_deg"] = float(sampled["cleave_angle_launch_1_deg"])
            if "path_adhesive_launch_1_um" in sampled:
                cfg_override["adh_len_px"] = float(sampled["path_adhesive_launch_1_um"]) / um
            if "path_interface1_um" in sampled:
                cfg_override["ferr_len_px"] = float(sampled["path_interface1_um"]) / um
            _build_reference_layout(cfg_override=cfg_override, sampled_params=sampled)

        csv_path, summary_path, rows = run_monte_carlo(
            scene=scene,
            build_scene_for_params=_build_for_params,
            emit_rays=lambda rng: _emit_rays(rng, fiber_rays_override=fiber_num_rays, cap=None),
            rng_manager=rng_manager,
            um_per_px=um,
            specs=DEFAULT_SPECS,
            trials=int(mc_trials),
            rays_cap=int(mc_rays_cap),
            base_seed=int(mc_base_seed),
            sigma_scale=float(mc_sigma_scale),
            exports_dir=exports_dir,
        )
        finite = [r.il_db for r in rows if not math.isinf(r.il_db) and not math.isnan(r.il_db)]
        if finite:
            mean_il = sum(finite) / len(finite)
            mc_last_msg = f"MC done: trials={len(rows)}  mean IL={mean_il:.2f} dB  → exports/ (CSV + summary)"
        else:
            mc_last_msg = f"MC done: trials={len(rows)}  (no finite IL) → exports/ (CSV + summary)"
    except Exception as ex:
        mc_last_msg = f"MC error: {type(ex).__name__}: {ex}"
...
def cb_export_detector_plots():
    """Export 2.5D beam profiles (heatmap + 3D surface) for each detector.

    Files are written to ./exports/detector_profiles/<timestamp>/
    """
    global export_last_msg
    try:
        base = os.path.join(os.path.dirname(__file__), "exports", "detector_profiles")
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = os.path.join(base, ts)
        um = float(interaction.um_per_px) if interaction.um_per_px else 1.0

        dets = [o for o in scene.objects if isinstance(o, Detector)]
        if not dets:
            export_last_msg = "Export: no detectors in the scene."
            return

        ok = 0
        skipped = 0
        for i, d in enumerate(dets):
            name = d.label or f"Detector{i+1}"
            paths = d.export_profile_plots(out_dir, um_per_px=um, name=name, bins=90)
            if paths:
                ok += 1
            else:
                skipped += 1

        export_last_msg = f"Exported profiles: {ok} detectors (skipped {skipped} empty) → {out_dir}"
    except Exception as ex:
        export_last_msg = f"Export error: {type(ex).__name__}: {ex}"
...
groups = [
    CollapsibleGroup(
        ...
        [
            ...
            Button(0, 0, 0, 0, "Clear Scene", cb_clear_scene),
    ...
    CollapsibleGroup(
        ...
        [
            ...
            Button(0, 0, 0, 0, "Export detector plots", cb_export_detector_plots),
            ...
            NumericField("Draw cap", lambda: draw_rays_cap, set_draw_cap_from_field, fmt="{:.0f}"),
    ...
    CollapsibleGroup(
        "Monte Carlo",
        [
            NumericField("Trials", lambda: mc_trials, set_mc_trials_from_field, fmt="{:.0f}"),
            NumericField("Rays cap", lambda: mc_rays_cap, set_mc_rays_cap_from_field, fmt="{:.0f}"),
            NumericField("Base seed", lambda: mc_base_seed, set_mc_base_seed_from_field, fmt="{:.0f}"),
            NumericField("Sigma scale", lambda: mc_sigma_scale, set_mc_sigma_scale_from_field, fmt="{:.3g}"),
            Button(0, 0, 0, 0, "Run Monte Carlo", cb_run_monte_carlo),
        ],
        expanded=False,
    ),
    CollapsibleGroup(
...
def _emit_rays(rng, fiber_rays_override: int | None = None, cap: int | None = None):
    ...
    for b in beams:
        r = b.emit(ray_id_start=ray_id, rng=rng)
        rays.extend(r)
        ray_id += len(r)
...
while running:
    ...
    for o in scene.objects:
        if isinstance(o, Detector):
            cx = (o.p1.x + o.p2.x) / 2
            cy = (o.p1.y + o.p2.y) / 2
            dname = (o.label + ": ") if getattr(o, "label", "") else ""
            label = font_small.render(f"{dname}{o.count} rays", True, (20, 20, 30))
    ...
    overlay_lines = [
        ...
        f"Draw rays = {'ON' if draw_rays_enabled else 'OFF'}  |  Draw cap = {draw_rays_cap}  |  Max rays = {max_rays}  |  Fiber rays = {fiber_num_rays}",
    ...
    if export_last_msg:
        overlay_lines.append(export_last_msg)
    if mc_last_msg:
        overlay_lines.append(mc_last_msg)
    ...
    if detectors:
        ...
        for i, d in enumerate(detectors, start=1):
            ...
            lbl = f" ({d.label})" if getattr(d, "label", "") else ""
            ps = len(getattr(d, "profile_samples", []))
            overlay_lines.append(f"Detector {i}{lbl}: {d.count} rays | P={p_det:.4f} | IL={_format_db(il_db)} dB | profile={ps}")
